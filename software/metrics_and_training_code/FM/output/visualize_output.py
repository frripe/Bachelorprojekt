# python3 FM/output/visualize_output.py
...
x1 = list(range(1, 48))
y1=[]
with open('FM/output/out_blur.txt', 'r') as file:
    y1 = file.read()
...
x2 = list(range(1, 35))
y2=[]
with open('FM/output/out_no_problems.txt', 'r') as file:
    y2 = file.read()
...
x0 = list(range(1, 252))
y0 = []
with open('FM/output/out_synth_blur.txt', 'r') as file:
    y0 = file.read()
    y0 = [float(i) for i in y0.split()]

print('FM mean = ' + str(round( min(m1, m2) + abs((m2-m1)/2), 3)))
...
plt.title("Frequency Domain metric, avg. speed = 0.104s") 
plt.xlabel("Image no.") 
plt.ylabel("Score")
plt.grid(True)
plt.legend()
plt.savefig('FM/output/output_FM.png')
...
plt.title('Frequency Domain metric')
plt.legend()
disp.plot()
plt.savefig('FM/output/output_FM_conf_mat.png')
...
plt.savefig('FM/output/output_FM_roc.png')
